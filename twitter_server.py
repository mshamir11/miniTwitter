...
Things to do
 ...
 1. Retweet
 2. Unfollow a particular user
 3. Delete a follower from his follower list
 4. Search for users
...
def feeds(client_socket,client_user_id):
    print("feeds")
    ...
    data =json.load(user_database)
    df = pd.DataFrame()
    for follower in data[client_user_id]['following']:
        print(follower)
    ...
    if response=='':
        ...
        for i in range(len(df)):
            if response==str(i+1):
                reTweet(client_socket,client_user_id,df.iloc[i]['tweet_id'])
        
def reTweet(client_socket,client_user_id,query_tweet_id):
    print("reTweet")
    sendMessage("\n\nRetweet\n\n Do you want to retweet this tweet:\n 1. Yes\n 2. No",client_socket)
    response =client_socket.recv(10).decode()
    if response=='1':
        sendMessage("Please type your re-tweet message to be posted",client_socket)
        retweet = client_socket.recv(1000).decode()
        tweet_table_df = pd.read_csv(TWEET_TABLE)
        ...
        old_tweet = tweet_table_df[tweet_table_df['tweet_id']==int(query_tweet_id)]['content'][int(query_tweet_id)-1]
        old_user_id = tweet_table_df[tweet_table_df['tweet_id']==int(query_tweet_id)]['user_id'][int(query_tweet_id)-1]
        old_user_name =data[str(old_user_id)]['user_name'] 
        old_post_date = tweet_table_df[tweet_table_df['tweet_id']==int(query_tweet_id)]['last_update_time'][int(query_tweet_id)-1]

        tweet = f"Retweet:\n{retweet}\n{old_tweet}\nBy: {old_user_name}\nPosted On:{old_post_date}" 
...
def postTweet(client_socket,user_id):
    sendMessage("\n\n\nPlease type your tweet to be posted",client_socket)
...
def searchByName(client_socket,client_user_id):
    print("search by name")
    sendMessage("\n\nSearch by Name: \n\n Please enter the name of the user.",client_socket)
    query_user_name = client_socket.recv(100).decode()
    user_id = open(USER_TO_ID,'r+')
    user_id_dic = json.load(user_id)
    if query_user_name in user_id_dic.keys():
        sendMessage(f"\n\nSearch Results: \n1. {query_user_name}. \n\n Press 0 to go to home page",client_socket)
    else:
        sendMessage(f"Sorry, the user does not exist in the database",client_socket)
    
    response = client_socket.recv(10).decode()
    
    if response=='1':
        individualUser(client_socket,str(user_id_dic[query_user_name]),query_user_name,client_user_id)
    else:
        homePage(client_socket,client_user_id)
...
def followUser(client_socket,target_user_id,target_user_name,client_user_id):
    ...
    if target_user_id in  data[client_user_id]['following']:
        sendMessage(f"\n\nYou have already following this user.\n 1. Previous Menu \n 2. Home Page \n",client_socket)
    else:
        ...
        user_database.write(json.dumps(data,indent=4))
        user_database.close()
        sendMessage(f"\n\nSuccessfully followed {target_user_name}.\n 1. Previous Menu \n 2. Home Page \n",client_socket)
    response = client_socket.recv(10).decode()
...
def unfollowUser(client_socket,target_user_id,target_user_name,client_user_id):
    ...
    if target_user_id not in  data[client_user_id]['following']:
        ...
        data[client_user_id]['following'].remove(target_user_id)
        data[target_user_id]['followers'].remove(client_user_id)
        user_database.close()
        user_database =open(USER_DATABASE, 'w')
        user_database.write(json.dumps(data,indent=4))
        user_database.close()
...
def trendingHashtags(client_socket,client_user_id):
    ...
    message += "\nTo view the posts enter the respective index of the hashtag. To go to the home page, press 0"  
    sendMessage(message,client_socket)
    response = client_socket.recv(10).decode()
    
    for i in range(5):
        if response ==str(i+1):
            viewHashtagPost(client_socket,hashtag_df_temp.iloc[i]['hashtag'],client_user_id)
            break

    if response=='0':
        homePage(client_socket,client_user_id)

def searchBynameHashtags(client_socket,client_user_id):
    print("search by name")
    sendMessage("\n\nSearch hashtag by keywords \n\n Please enter a hashtag",client_socket)
    query_hashtag = client_socket.recv(100).decode()
    hashtag = open(HASHTAG,'r+')
    hashtag_dic = json.load(hashtag)
    if query_hashtag in hashtag_dic.keys():
        viewHashtagPost(client_socket,query_hashtag,client_user_id)
    else:
        sendMessage("The hashtag does not exist in the database. Press 0 to go to homepage",client_socket)
...
def hashtags(client_socket,client_user_id):
    sendMessage("\n\nHashtags Page: \n\n 1. Top trending Hashtags\n 2. Search Hastag \n\nPress 0 to go to homepage",client_socket)
    response = client_socket.recv(10).decode()
    if response=='1':
        trendingHashtags(client_socket,client_user_id)
    elif response=='2':
        searchBynameHashtags(client_socket,client_user_id)
...
def listOfmyFollowing(client_socket,client_user_id):
    user_database =open(USER_DATABASE, 'r+')
    data =json.load(user_database)
    user_database.close()
    message="\n\nList of following:\n\n"
    ...
    for i,item in enumerate(data[client_user_id]['following']):
...
def listOfmyFollowers(client_socket,client_user_id):
    user_database =open(USER_DATABASE, 'r+')
    data =json.load(user_database)
    user_database.close()
    message="\n\nList of followers:\n\n"
    ...
    for i,item in enumerate(data[client_user_id]['followers']):
        if item!=client_user_id:
            message += f"{i+1}. {data[item]['user_name']} \n"
...
def individualUser(client_socket,target_user_id,target_user_name,client_user_id):
    ...
    if response=='1':
        ...
        listOfFollowers(client_socket,str(target_user_id),target_user_name,client_user_id)
...
def homePage(client_socket,user_id):
    ...
    if response=='1':
        ...
        listOfmyFollowers(client_socket,user_id)
    elif response=='5':
        listOfmyFollowing(client_socket,user_id)
